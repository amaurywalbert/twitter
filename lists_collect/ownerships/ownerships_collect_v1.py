# -*- coding: latin1 -*-
################################################################################################
# Script para coletar listas criadas pelos usuários
#
...
################################################################################################
#
# Verifica quais os limites ainda disponíveis para consumo da API do Twitter
#
################################################################################################
...
def collect_lists_ownerships(owner):	
	...
	try:
		...
		lists_file = open(owner+"_lists_collect_"+agora+".txt", 'w')
		
		
		i = 0
		
		for list in lists_ownerships:
			i = i + 1
			
			# Na tela, imprime o resumo de cada lista...
			print(owner,i,list.id,list.created_at,list.full_name,list.member_count)
