...
#		ERRO - Tentando salvar os arquivos no formato JSON de maneira mais simples e com uso do método api.Cursor
...
def get_lists_ownerships(owner):
	...
	try:
		...
		for item in tweepy.Cursor(api.get_lists_ownerships,id=owner,wait_on_rate_limit=True, count=1000).items():						
			for lista in item:
				#data = json.load(lista)
				print(lista)
    			
				# ... no arquivo, imprime a lista inteira.
				lists_file.write(json.dumps(lista._json)+"\n")
