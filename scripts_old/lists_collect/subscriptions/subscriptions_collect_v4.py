# -*- coding: latin1 -*-
################################################################################################
# Script para coletar listas subscritas pelas usuários
#
...
def get_lists_subscriptions(user):
	print("Recuperando listas de: "+user)
	...
	try:
		...
		lists_file = open("data/subs_lists_collect_"+agora+"_"+user+".txt", 'w')
		...
		for page in tweepy.Cursor(api.lists_subscriptions,id=user,wait_on_rate_limit=True, count=1000).pages():						
			for list in page:
				...
				print(user,i,list.id,list.created_at,list.name,list.member_count)
		...
		subs_list_err = open(user+"_lists_collect_"+agora+".subs_list_err", "a+") # Abre o arquivo para gravação no final do arquivo
...
#	accounts = arquivo.readlines()
	...
	for account in accounts:
		get_lists_subscriptions(account)
#	arquivo.close()
