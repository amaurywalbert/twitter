...
def save_timeline(j,k,l,user):
	...
	if timeline:	
		try:
			...
			dictionary[user] = user									# Insere o usuário coletado na tabela em memória
			i +=1
			print ("Ego nº "+str(j)+" - Alter ("+str(k)+"/"++str(l)+"): "+str(user)+" coletado com sucesso. "+str(t)+" tweets. Total de usuários coletados: "+str(i))
...
def main():
	...
	for file in os.listdir(egos_dir):					# Verifica a lista de egos coletados e para cada um, busca os amigos dos alters listados no arquivo do ego.
		...
		friends_list = read_arq_bin(egos_dir+file)	# Lista de alters (friends) de um determinado ego
		l = len(friends_list)								# Exibe o tamanho/quantidade de amigos na lista de amigos do ego
		for friend in friends_list:
			k+=1
			if not dictionary.has_key(friend):
				save_timeline(j,k,l,friend)				#Inicia função de busca
			else:
				print "Usuário: "+str(friend)+" já coletado!"
	print
	print("######################################################################")
...
data_dir = "/home/amaury/coleta/timeline_collect/egos_and_friends/json/" ################### Diretório para armazenamento dos arquivos
error_dir = "/home/amaury/coleta/timeline_collect/egos_and_friends/error/" ################# Diretório para armazenamento dos arquivos de erro
wait = 5
