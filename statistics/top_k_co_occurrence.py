...
def jaccard_modified(set1,set2):
	if len(set1) > 0:
		...
		result = intersection/float(len(set1))				#Tamanho da interseção dos conjuntos sobre o tamanho do ranking.
	else:
		result = 0
									
	return result													#... verifica qual o percentual dos top_k estão presentes nas demais camadas.
...
#
######################################################################################################################################################################
def calc_co_occurrence(alters_set,top_k):
	pairs = {}			

	for k,v in top_k.iteritems():
		set1 = v
		for j, x in alters_set.iteritems():
			...
			for ego,v in dictionary.iteritems():
				...
				for net in nets:
					...
					if not os.path.exists(graphs_dir):
						...
						alters_set[layer],top_k[layer] = get_top_k(ego,graphs_dir+str(ego)+".edge_list")			# Recupera os alters da camada n para ego_id

				pairs = calc_co_occurrence(alters_set,top_k) 
				save_file(ego,pairs,out_file)																						# Salvar arquivo texto			
				top_k_dataset[ego] = pairs
					
				print ("Ego: "+str(i)+" - Top-K Co-Occurrence: "+str(top_k_dataset[ego]))
				print
