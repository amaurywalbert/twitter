...
def main():
	...
	for file in os.listdir(friends_dir):
		...
		if (len(friends_set) >= k) and (len(followers_set) >= k) and (len(retweets_set) >= k) and (len(likes_set) >= k) and (len(mentions_set) >= k):
			intersection.append(ego_id)
		else:
			print (str(i)+" - "+str(ego_id)+" - abaixo do threshold")
			print ("FRIENDS: "+str(len(friends_set))+" - FOLLOWERS: "+str(len(followers_set))+" - RETWEETS: "+str(len(retweets_set))+" - LIKES: "+str(len(likes_set))+" - MENTIONS: "+str(len(mentions_set)))
			print
	...
	fim = time.time()
	print("######################################################################")
	print "Script V_1.0 finalizado! Tempo de Execução: ",fim-ini
	print("######################################################################\n")
#####################################################################################################################################################################
#
# INÍCIO DO PROGRAMA
...
k = 200 										# threshold - Conjunto mínimo - 10,20,30,40,50, 'full'
######################################################################################################################
