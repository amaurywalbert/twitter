	...
	if not os.path.exists(source_dir):
		...
		for i in range(10):
				...
				for file in os.listdir(graphs_dir):																# Para cada ego da rede $net
					if not os.path.isfile(graphs_dir+file):
						...
						if not os.path.isfile(source_dir+str(ego_id)+".txt"):
							...
							_users_by_list,_users_by_list_average, _lists_by_ego, _ego_jaccard, _full_lits_jaccard = lists_verify(str(source_dir)+str(ego_id)+".txt", alters_set)
			...
			if not os.path.exists(output):
				os.makedirs(output)			
			graphics.histogram(users_by_list_average,output,title=str(network)+' - Users by Lists - Average', xaxis='Users', yaxis='Lists')
			graphics.histogram(users_by_list,output,title=str(network)+' - Users by Lists - Full Egos', xaxis='Users', yaxis='Lists')
			graphics.histogram(lists_by_ego,output,title=str(network)+' - Lists by Ego', xaxis='Lists', yaxis='Egos')
			grapgics.histogram(ego_jaccard,output,title=str(network)+' - Jaccard Modified by Egos', xaxis='Jaccard Modified', yaxis='Egos')
			grapgics.histogram(full_lists_jaccard,output,title=str(network)+' - Jaccard Modified by Lists', xaxis='Jaccard Modified', yaxis='Lists')
...
#			overview = {'users_by_list':USERS_BY_LIST,'users_by_list_avg':USERS_BY_LIST_AVERAGE,'lists_by_ego':LISTS_BY_EGO,'jaccard_avg':JACCARD_AVG,'ego_jaccard':ego_jaccard}
			overview = {'users_by_list':USERS_BY_LIST,'users_by_list_avg':USERS_BY_LIST_AVERAGE,'lists_by_ego':LISTS_BY_EGO,'jaccard_by_ego_avg':JACCARD_AVG, 'jaccard_by_lists':FULL_LISTS_JACCARD}
