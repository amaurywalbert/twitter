...
def save_error(user,reason):
	...
	error={}
	with open(error_dir+"favorites_collect_wait_APP2.err", "a+") as outfile:								# Abre o arquivo para gravação no final do arquivo
...
####				
				...
				if os.path.exists(data_dir+str(user)+".dat"):
					...
					print ("Ego nº: "+str(j)+" - Alter ("+str(k)+"/"+str(l)+"): "+str(user)+" - Favoritos não recuperados... Arquivo excluído!")
