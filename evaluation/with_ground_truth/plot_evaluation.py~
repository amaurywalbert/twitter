...
def plot_full_with_singletons(output,data1,data2,metric,alg):
	...
	p1=plt.bar(ind-0.1,y,width,color="blue", label="Grafo COM ego - "+str(dataset[1][3]))
	p2=plt.bar(ind,z,width,color="green", label="Grafo SEM ego - "+str(dataset[2][3]))
...
#
######################################################################################################################################################################
def plot_full_metrics_with_singletons(output,dataset1,dataset2,alg):
	title = "Avaliação das redes usando o algoritmo "+str(alg)
	print ("\n##################################################\n")
	print ("Gerando Gráfico Completo...")
	...
	for array in dataset1:
		...
		data1 = [value,interaction,std,threshold]
		dataset[i] = data1
	...
	for array in dataset2:
		...
		data2 = [value,interaction,std,threshold]
		dataset[i] = data2
	...
	y1 = np.array(dataset[3][0])								# recebe os valores COM ego
	z1 = np.array(dataset[4][0])								# recebe os valores SEM ego
	...
	p4=plt.bar(ind+0.2,z1,width,color="green", label="JACCARD - Grafo SEM ego - Threshold - "+str(dataset[4][3]))
...
################################################################################################  MANTER -- Dá pra exportar a tabela depois...
	...
	trace3 = go.Bar(x = dataset[3][1], y = dataset[5][0], error_y=dict(type='data',array=dataset[3][2], color='#E6842A', visible=True), name="JACCARD - Grafo COM ego - Threshold - "+str(dataset[3][3]), marker=dict(color='darkgreen'))
	trace4 = go.Bar(x = dataset[4][1], y = dataset[6][0], error_y=dict(type='data',array=dataset[4][2], color='#E6842A', visible=True), name="JACCARD - - Grafo Sem ego - Threshold - "+str(dataset[4][3]), marker=dict(color='seagreen'))
