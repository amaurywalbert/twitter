...
def calculate_alg(communities,output,singletons,net,graphs,uw,ud,g_type,alg):
	...
	if alg == "oslom":
		...
		threshold = 51
		print ("Threshold para COPRA ainda não definido")
		sys.exit()
	else:
		threshold = 51
		print ("Algoritmo não reconhecido: "+str(alg))
		sys.exit()		
...
#######################################################################
	...
	for i in range(10):								# Para cada rede-ego gerada
		...
		calculate_alg(communities2,output2,singletons,net,graphs2,uw,ud,g_type,alg)
