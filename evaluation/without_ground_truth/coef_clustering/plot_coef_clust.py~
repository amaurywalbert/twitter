...
def plot_bars_full_without_singletons(output,data1,data2,metric,alg):
	...
	p1=plt.bar(ind,y,width,color="blue", label="Grafo COM ego - Threshold - "+str(dataset[1][3]))

	p2=plt.bar(ind+0.1,z,width,color="green", label="Grafo SEM ego - Threshold - "+str(dataset[2][3]))
...
######################################################################################################################################################################
# Plota Gráficos dos dados... Desconsiderando se há ou não singletons - Usando BoxPlot
######################################################################################################################################################################
def box_plot_without_singletons(output,data1,metric,alg):
	...
	for k, v in data1.iteritems():
		...
		if k == 'n1':
			...
			print ("Valor incorreto para nome da rede-ego: "+str(k))
	output = output+str(metric)+"/"
	if not os.path.exists(output):
		os.makedirs(output)
################################################################################################  MANTER -- Dá pra exportar a tabela depois...
	...
	plotly.offline.plot(fig, filename=output+str(alg)+"_box_plot.html",auto_open=False)
	print ("Arquivo salvo: "+str(output)+str(alg)+"_box_plot.html")
