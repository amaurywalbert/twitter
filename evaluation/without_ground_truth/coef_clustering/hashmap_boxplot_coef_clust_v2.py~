...
######################################################################################################################################################################
##		Status - Versão 2 - Script para plotar a coef_clust em BoxPlot
## 
######################################################################################################################################################################
			...
			if os.path.isdir(dataset+directory):
				...
				for file in os.listdir(dataset+directory):
					with open(dataset+directory+"/"+file, 'r') as f:
						...
						if data is not None:
							_metric = []
							for k,v in data['coef_clust_data'].iteritems():
								_metric.append(v[0])
...
######################################################################
	...
	if data1 is not None:
		plot_coef_clust.box_plot_without_singletons(output,data1,metric,alg)			# Só para INFOMAP
	else:
