...
public class DisjointCommunityQuality {
...
//		System.out.println("isUnweighted = " + isUnweighted + ", isUndirected = " + isUndirected);
		...
		double[] qualities = DisjointCommunityQuality.computeQualityWithoutGroundTruth(
...
/*
		...
		qualities = DisjointCommunityQuality.computeInformationEntropyMetrics(
				discoveredCommunityFile, groundTruthCommunityFile);
		System.out.println("VI = " + qualities[0] + ", NMI = " + qualities[1]);

		qualities = DisjointCommunityQuality.computeClusterMatchingMetrics(
				discoveredCommunityFile, groundTruthCommunityFile);
		System.out.println("F-measure = " + qualities[0] + ", NVD = "
				+ qualities[1]);

		qualities = DisjointCommunityQuality.computeIndexMetrics(
				discoveredCommunityFile, groundTruthCommunityFile);
...
*/	
	...
	public static double[] computeQualityWithoutGroundTruth(String networkFile,
		...
		double[] weights = DisjointCommunityQuality.getNetwork(networkFile,
				isUnweighted, isUndirected, outNet);
		double totalWeight = weights[0];

		// Get communities and their nodes
		Map<Integer, Set<Integer>> communities = DisjointCommunityQuality
		...
		Map<Integer, Integer> nodeCommunities = DisjointCommunityQuality
...
//			if (nodeCommunities.get(nodeId) == null){														// Vértice que não foi adicionado a nenhuma comunidade  
...
//			}
	...
	public static double[] computeInformationEntropyMetrics(
		...
		List<Set<Integer>> discoveredCommunities = DisjointCommunityQuality
				.getCommunities(disCommunityFile);
		List<Set<Integer>> realCommunities = DisjointCommunityQuality
		...
		for (int i = 0; i < realsize; ++i) {
			...
			for (int j = 0; j < exsize; ++j) {
				Set<Integer> exCommunity = discoveredCommunities.get(j);
				double nj = exCommunity.size();
				double nij = DisjointCommunityQuality.getIntersectionNumber(
	...
	public static double[] computeClusterMatchingMetrics(
		...
		for (int j = 0; j < realsize; ++j) {
			...
			for (int i = 0; i < exsize; ++i) {
				...
				double nji = DisjointCommunityQuality.getIntersectionNumber(
						realCommunity, exCommunity);
	...
	public static double[] computeIndexMetrics(String disCommunityFile,
		...
		Map<Integer, Integer> disNodeCommunities = DisjointCommunityQuality
				.getNodeCommunities(disCommunityFile);
		Map<Integer, Integer> realCommunities = DisjointCommunityQuality
