...
##					Teste - Autenticação através de várias contas do twitter.
##
## 
################################################################################################
...
def autentication(aut):	
	...
	print ("Autenticando usando chave número: "+str(aut))

	api = tweepy.API(auths[aut])
...
def members_lists(list_id,api):
	print("Coletando membros da lista: "+str(list_id))
	members_file = open("data/users_collected.txt", 'a+')								# Arquivo com os seeds (membros das listas selecionadas serão adicionados ao final do arquivo user collect para continuar o processo de busca
	print	
...
#		for page in tweepy.Cursor(api.list_members,list_id=list_id,wait_on_rate_limit=True,wait_on_rate_limit_notify=True,count=5000).pages():
		for page in tweepy.Cursor(api.list_members,list_id=list_id,wait_on_rate_limit_notify=True,count=5000).pages():
			for member in page:
				if check(member.id,members_file):								#Verifica se o usuário já foi adicionado no arquivo de membros coletados.
	...
	except tweepy.RateLimitError:																# Verifica se o erro ocorreu por limite excedido, faz nova autenticação e chama a função novamente.
		global aut
		aut = aut+1
		api = autentication(aut)
		members_lists(list_id,api)		
...
def search_lists(user,api):
	...
	try:
		...
		if (len(lists) > 1):
			...
			for list in lists:
				if check(list.id,lists_id):													#Verifica se a lista já foi adicionada no arquivo de listas.
					...
					members_lists(list.id,api)														#Função para recuperar os membros da lista
		...
		users_verified.writelines(str(user))									# Salva o usuário no arquivo de users já verificados.
		users_verified.close()	
	except tweepy.RateLimitError:																# Verifica se o erro ocorreu por limite excedido, faz nova autenticação e chama a função novamente.
		global aut		
		aut = aut+1
		api = autentication(aut)
		search_lists(user,api)
...
def main(api):
	...
	users_verified.close()
	try:
		users_collected = open('data/users_collected.txt','r') 							#Testando com o id do user - Dever conter a lista dos seeds
	...
	while not eof:																					#Enquanto não for final do arquivo
		...
		if (account == ''):																		#Se id for igual a vazio é porque chegou ao final do arquivo.
			...
			if check(account,users_verified):													#testa se o usuário já foi verificado, consultando o arquivo correspondente.
				...
				print
				print("####################################################################################################")			
				search_lists(account,api)																#Inicia função de busca das listas e coleta dos membros
				print("####################################################################################################")
...
#
################################################################################################
aut = 0																									#variável global que armazena dados da autenticação

api = autentication(aut)

# Verifica se eh para executar o metodo main()
if __name__ == "__main__": main(api)