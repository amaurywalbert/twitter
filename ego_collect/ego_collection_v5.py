...
def members_lists(list_id):
	...
	try:
		...
		for page in tweepy.Cursor(api.list_members,list_id=list_id,wait_on_rate_limit=True,wait_on_rate_limit_notify=True,count=5000).pages():
			for member in page:
				users_collect = open("data/users_collect.txt", 'r')											# Arquivo com os seeds (membros das listas selecionadas serão adicionados ao final do arquivo user collect para continuar o processo de busca
				if check(member.id,users_collect):								#Verifica se o usuário já foi adicionado no arquivo de membros coletados.
					print (str(member.id)+" já adicionado! Continuando...")
				else:					
					members.append(str(member.id)+"\n")
				users_collect.close()
		members_file = open("data/users_collect.txt", 'a+')
...
def main():
	#users_verified = open('data/users_verified.txt','a+')							#Arquivo para armazenar a lista de usuários já verificados.
	#users_verified.close()
	users_collect = open('data/users_collect.txt','r') 							#Testando com o id do user
	eof = False
