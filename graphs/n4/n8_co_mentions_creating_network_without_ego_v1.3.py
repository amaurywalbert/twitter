...
######################################################################################################################################################################
##		Status - Versão 1 - Criar rede N8 (co-mentions) a partir dos dados coletados e de acordo com as instruções a seguir:
...
## # INPUT:
##		- Lista de Egos (egos)
##		- Conjunto de Mentioneds (alters) de cada Ego - Formação do conjunto de Alters
##		- Conjunto de Mentioneds de cada Alter - verificação do CSJ entre os conjuntos de pares de vértices
##
## # ALGORITMO
##					0 - Para cada ego[i]:
##					1 - 	Inicializa o ego_[i] e todos os mencionados do ego (alters[i][n]) como vértices de um grafo - (tabela hash - ego+alters - vertices)
...
def read_arq_bin(file):
	with open(file, 'r') as f:	 
		...
		while f.tell() < tamanho:
			...
			mentioned = user_struct.unpack(buffer)
			mentions_set.add(long(mentioned[0]))
	return mentions_set
...
#	vertices[ego] = alters_set															# Adiciona o Ego ao conjunto de vértices
	for alter in alters_set:
		try:
			alters_mentions = read_arq_bin(alters_dir+str(alter)+".dat")# Chama função para converter o conjunto de autores de likes dos alters do formato Binário para uma lista do python
			vertices[alter] = alters_mentions									# Adiciona conjunto de dados (mentions_ids) do alter à tabela hash
...
def main():
	...
	for file in os.listdir(egos_dir):										# Para cada arquivo de Ego no diretório
		...
		if not dictionary.has_key(ego):
			alters_set = read_arq_bin(egos_dir+file)							# Chama função para converter o conjunto de menções do ego do formato Binário para uma lista do python
...
output_dir = "/home/amaury/graphs/n8/graphs_without_ego/" ################# Diretório para armazenamento dos arquivos das listas de arestas 
output_dir_errors = "/home/amaury/graphs/n8/error_without_ego/" ########## Diretório para armazenamento dos erros
