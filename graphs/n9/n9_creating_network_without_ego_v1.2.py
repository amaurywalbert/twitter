...
######################################################################################################################################################################
##		Status - Versão 1 - Criar rede N9 (seguidores) a partir dos dados coletados e de acordo com as instruções a seguir:
...
## # INPUT:
##		- Lista de Egos (egos)
##		- Lista Followers (alters) de cada Ego - Formação do conjunto de Alters
##		- Lista Followee (followees) de cada Alter (ids) (amigos dos seguidores)
##
## # ALGORITMO
##					0 - Para cada ego[i]:
##					1 - 	Inicializa o ego_[i] e todos os seus seguidores (alters[i][n]) como vértices de um grafo - (tabela hash - ego+alters - vertices)
##					2 - 	Cria uma aresta direcionada de cada alter[i][n] para o ego[i]
...
def read_arq_bin(file):															# Função recebe o arquivo binário
	with open(file, 'r') as f:	 
		...
		while f.tell() < tamanho:
			...
			user = user_struct.unpack(buffer)
...
#		G.add_edge(alter,ego)													# Cria uma aresta entre cada ALTER e o EGO - Adiciona alter com arquivo em branco
	i = 0
	for alter in alters_list:
		i+=1	
		try:
...
def main():
	...
	for file in os.listdir(egos_dir):										# Para cada arquivo de Ego no diretório
		...
		if not dictionary.has_key(ego):		
			...
			n_followers = len(alters_list)											# Variável que armazena o tamanho da lista do usuário corrente
			
			print("######################################################################")
			print ("Construindo grafo do ego n: "+str(l)+" - Quantidade de seguidores: "+str(n_followers))
			...
			if partial_missing:
				...
				overview = {'ego':ego,'n_followers':n_followers,'errors':len(partial_missing),'missing':partial_missing}		# cria dicionário python com informações sobre a criação do grafo do ego corrente
...
output_dir = "/home/amaury/graphs/n9/graphs_without_ego/" ################# Diretório para armazenamento dos arquivos das listas de arestas 
output_dir_errors = "/home/amaury/graphs/n9/error_without_ego/" ########## Diretório para armazenamento dos erros
output_overview = "/home/amaury/graphs/n9/overview_without_ego.json" ########## Diretório contendo arquivos com informações sobre a construção das redes. 
