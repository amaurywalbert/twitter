...
#    term3 = x_l * (math.log(1 / (1 - p)) - sum(p**d / d for d in range(1, f + 1)))    
    ...
    term1 = s * sum(p**d / d for d in range(int(s + 1), int(f + 1)))					##alterado por amaury
    term2 = l * sum(p**d / d for d in range(int(l + 1), int(f + 1)))					##alterado por amaury
    term3 = x_l * (math.log(1 / (1 - p)) - sum(p**d / d for d in range(1, int(f + 1))))					##alterado por amaury
