...
##					Versão 2 - Versão com imagens geradas em preto/branco e com detalhes em símbolos diferentes para diferenciar os dados. 
##
##
...
def make_plot(metric,df,output):
	...
	patterns = [ "/" , "|" , "o", ".", "*" ]												# Padrões de textura para cada layers
#	patterns = [ "/" , "\\" , "|" , "-" , "+" , "x", "o", "O", ".", "*" ]						# Padrões de textura para cada layers
	...
	# Create a bar charts
	ax.bar(bar_l, rl_rel, label='rl', alpha=0.9, width=bar_width, color='white', edgecolor='black')
	ax.bar(bar_l, as_rel, bottom=rl_rel, label='as', alpha=0.9, width=bar_width, color='white', edgecolor='black', hatch=patterns[0])
	ax.bar(bar_l, am_rel, bottom=[i+j for i,j in zip(rl_rel, as_rel)], label='am', alpha=0.9, width=bar_width, color='white', edgecolor='black', hatch=patterns[1])
	ax.bar(bar_l, al_rel, bottom=[i+j+k for i,j,k in zip(rl_rel, as_rel,am_rel)], label='al', alpha=0.9, width=bar_width, color='white', edgecolor='black', hatch=patterns[2])
	ax.bar(bar_l, rm_rel, bottom=[i+j+k+l for i,j,k,l in zip(rl_rel, as_rel, am_rel, al_rel)], label='rm', alpha=0.9, width=bar_width, color='white', edgecolor='black', hatch=patterns[3])
	
	ax.bar(bar_l, ar_rel, bottom=[i+j+k+l+m for i,j,k,l,m in zip(rl_rel, as_rel, am_rel, al_rel,rm_rel)], label='ar', alpha=0.9, width=bar_width, color='darkgray', edgecolor='black')
	ax.bar(bar_l, lm_rel, bottom=[i+j+k+l+m+n for i,j,k,l,m,n in zip(rl_rel, as_rel, am_rel, al_rel,rm_rel,ar_rel)], label='lm', alpha=0.9, width=bar_width, color='darkgray', edgecolor='black', hatch=patterns[0])
	ax.bar(bar_l, ls_rel, bottom=[i+j+k+l+m+n+o for i,j,k,l,m,n,o in zip(rl_rel, as_rel, am_rel, al_rel,rm_rel,ar_rel,lm_rel)], label='ls', alpha=0.9, width=bar_width, color='darkgray',edgecolor='black', hatch=patterns[1])
	ax.bar(bar_l, ms_rel, bottom=[i+j+k+l+m+n+o+p for i,j,k,l,m,n,o,p in zip(rl_rel, as_rel, am_rel, al_rel,rm_rel,ar_rel,lm_rel,ls_rel)], label='ms', alpha=0.9, width=bar_width, color='darkgray', edgecolor='black', hatch=patterns[2])
	ax.bar(bar_l, rs_rel, bottom=[i+j+k+l+m+n+o+p+q for i,j,k,l,m,n,o,p,q in zip(rl_rel, as_rel, am_rel, al_rel,rm_rel,ar_rel,lm_rel,ls_rel,ms_rel)], label='rs', alpha=0.9, width=bar_width, color='darkgray', edgecolor='black', hatch=patterns[3])
