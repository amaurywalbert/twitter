...
##									Acrescentei o cálculo da densidade. 
## 
######################################################################################################################################################################
...
def net_structure(dataset_dir,output_dir,net,IsDir, weight):
	print("\n######################################################################\n")
	if os.path.isfile(str(output_dir)+str(net)+"_net_struct_density.json"):
		print ("Arquivo já existe: "+str(output_dir)+str(net)+"_net_struct_density.json")
	else:
...
#####################################################################################
				...
				w = float(n_edges)/(float(n_nodes)*(float(n_nodes)-1))			# Calcular a densidade da rede
				dens.append(w)
				density[ego_id] = w	
...
#####################################################################################
#
#				print	n[i-1], e[i-1], d[i-1], cc[i-1], bc_n[i-1], bc_e[i-1], dens[i-1]
#				print 
#####################################################################################		
...
#			f.write(json.dumps(overview_basics))
		...
		with open(str(output_dir)+str(net)+"_net_struct_density.json", 'w') as f:
